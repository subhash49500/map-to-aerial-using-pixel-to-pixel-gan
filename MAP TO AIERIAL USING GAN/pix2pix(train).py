...
prev_time = time.time()
no_batchs=len(dataloader)
for epoch in range(opt.epoch, opt.n_epochs):
    running_results = {'d_loss': 0, 'g_loss': 0}        
    for i, batch in enumerate(dataloader):
        ...
        if i == no_batchs-1:
            ...
            print(
            str(timenow)+"[Epoch %d/%d] [D loss: %f] [G loss: %f]"
            % (
                epoch,
                opt.n_epochs,
                running_results['d_loss'] / no_batchs,
                running_results['g_loss'] / no_batchs 
            )
        )
